#!/usr/bin/env python3
"""
FHub Quality + Batch Backfill Script
=========================================
Objective:
...
Usage:
  ...
  python3 scripts/backfill_quality.py --db /tmp/fhub_backfill.db --dry-run

  # Apply changes and push back:
  python3 scripts/backfill_quality.py --db /tmp/fhub_backfill.db --push

Options:
  --db PATH       Path to fhub.db (required)
...
HEADERS = {
    ...
    'User-Agent': 'Mozilla/5.0 (compatible; FHub/2.0)',
}
...
async def main():
    ...
    if args.push and not args.dry_run:
        ...
        cmds = [
            'ssh root@pve-remote "pct exec 112 -- docker stop fhub"',
            f'ssh root@pve-remote "pct exec 112 -- bash -c \'cat > /var/lib/docker/volumes/fhub_appdata/_data/data/fhub.db\'" < {db_path}',
            'ssh root@pve-remote "pct exec 112 -- docker start fhub"',
        ]
        # Simpler: use docker cp via stdin
        r = subprocess.run(
            f'ssh root@pve-remote "pct exec 112 -- docker cp - fhub:/appData/data/fhub.db" < {db_path}',
            shell=True, capture_output=True, text=True
        )
        if r.returncode == 0:
            print('  ✓ DB pushed — restarting container...')
            subprocess.run('ssh root@pve-remote "pct exec 112 -- docker restart fhub"', shell=True)
            print('  ✓ Container restarted')
        else:
            # Fallback: copy to tmp path inside container then move
            print(f'  ⚠  Direct copy failed ({r.stderr.strip()}), trying cat method...')
            r2 = subprocess.run(
                f'cat {db_path} | ssh root@pve-remote "pct exec 112 -- docker exec -i fhub bash -c '
                f'\'cat > /tmp/fhub_new.db && cp /tmp/fhub_new.db /appData/data/fhub.db\'"',
                shell=True, capture_output=True, text=True
            )
            if r2.returncode == 0:
                subprocess.run('ssh root@pve-remote "pct exec 112 -- docker restart fhub"', shell=True)
                ...
                print(f'    ssh root@pve-remote "pct exec 112 -- docker cp /tmp/fhub_backfill.db fhub:/appData/data/fhub.db"')
                print(f'    ssh root@pve-remote "pct exec 112 -- docker restart fhub"')
