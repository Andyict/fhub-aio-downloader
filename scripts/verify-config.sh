...
echo -e "${BLUE}╔═══════════════════════════════════════╗${NC}"
echo -e "${BLUE}║   FHub Configuration Checker     ║${NC}"
echo -e "${BLUE}╚═══════════════════════════════════════╝${NC}"
echo ""
...
if grep -q "nas2nd/fhub" README.md; then
    echo -e "${GREEN}✓${NC}"
else
    echo -e "${RED}✗${NC}"
...
if grep -q "ghcr.io/nas2nd/fhub" docker-compose.production.yml; then
    echo -e "${GREEN}✓${NC}"
else
    echo -e "${RED}✗${NC}"
...
if grep -q "nas2nd/fhub" install.sh; then
    echo -e "${GREEN}✓${NC}"
else
    echo -e "${RED}✗${NC}"
...
if [ -f "../.github/workflows/docker-publish.yml" ]; then
    if grep -q "Andyict" ../.github/workflows/docker-publish.yml; then
        echo -e "${GREEN}✓${NC}"
    else
        echo -e "${YELLOW}⚠${NC}"
...
if grep -q "Andyict" Dockerfile; then
    echo -e "${GREEN}✓${NC}"
else
    echo -e "${YELLOW}⚠${NC}"
...
if [ $ERRORS -eq 0 ] && [ $WARNINGS -eq 0 ]; then
    ...
    echo ""
    echo -e "${GREEN}Your FHub project is ready to publish!${NC}"
    echo ""
    echo -e "Next steps:"
