...
use crate::hosts::fshare::FshareHandler;
...
async fn add_account(
    ...
    if let Some(handler) = state.host_registry.get_handler_for_url("https://fshare.vn/file/test") {
        ...
        match handler.check_account_status().await {
            ...
            Err(e) => {
                // Some FShare status endpoints are flaky even after credentials are accepted.
                // Keep the credentials active, but mark the account as pending verification instead of rejecting login.
                tracing::warn!("[Accounts] add_account: status check failed after saving credentials: {}", e);
                let _ = state.db.save_setting("fshare_rank", "UNVERIFIED");
                let _ = state.db.save_setting("fshare_valid_until", "0");
                return Ok(Json(ActionResponse {
                    success: true,
                    message: Some(format!("Account saved. VIP verification pending: {}", e)),
                }));
            }
        }
    }
...
async fn remove_account(
    ...
    // Resolve the active account from DB first because the UI writes credentials
    // there. Falling back to config keeps old config.toml installs compatible.
    ...
    // Clear DB-backed credentials used by FshareHandler/list_accounts.
    let db_results = [
        state.db.save_setting("fshare_email", ""),
        state.db.save_setting("fshare_password", ""),
        state.db.save_setting("fshare_rank", ""),
        state.db.save_setting("fshare_valid_until", "0"),
    ];

    if let Some(err) = db_results.into_iter().find_map(Result::err) {
        tracing::error!("Failed to clear FShare account from database: {}", err);
        return Json(ActionResponse {
            success: false,
            message: Some(format!("Failed to clear account: {}", err)),
        });
    }

    // Reset config.toml too for compatibility with deployments that still boot
...
async fn verify_account(
    ...
    // Verify with a temporary handler so existing saved credentials/session are
    // not overwritten. The handler falls back to this cloned config because no
    // DB is attached to it.
    let mut fshare_config = state.config.fshare.clone();
    fshare_config.email = payload.email.clone();
    fshare_config.password = payload.password;
    fshare_config.session_id = None;

    let verifier = FshareHandler::new(fshare_config, crate::hosts::create_shared_client());
    match verifier.check_account_status().await {
        Ok(status) => {
            let rank = if status.premium { "VIP" } else { "FREE" };
            Json(ActionResponse {
                success: status.can_download,
                message: Some(format!("Verified {} account: {}", rank, status.account_email)),
            })
        }
        Err(e) => Json(ActionResponse {
            success: false,
