...
async fn enhanced_search(
    ...
    // FHUB reuses the shared HTTP client for connection pooling and TLS reuse.
    let client = (*state.http_client).clone();
        
    let parsed_query = smart_parse(&params.q);
    
    // FHUB deep search keeps a broad upstream result window for richer client-side selection.
    let url = format!("https://api.timfshare.com/api/v1/string-query-search?query={}", urlencoding::encode(&params.q.trim()));
    ...
    let backend_version = "fhub-search-v3.3".to_string();
    ...
    match resp {
            ...
            if let Ok(data) = r.json::<serde_json::Value>().await {
                let mut results: Vec<SearchResult> = data["data"].as_array().map(|arr| {
                    arr.iter().map(|item| {
                        ...
                        // FHUB only penalizes very weak title matches to avoid hiding useful variants.
                ...
                // FHUB keeps only playable media-like files in the operator search surface.
                results.retain(|r| is_media_file(&r.original_filename));
                
                // Rank best activity candidates first.
                results.sort_by(|a, b| b.score.cmp(&a.score));
                
                // FHUB enriches a broad top slice before pagination so the UI can group and filter locally.
                let enrichment_pool_size = results.len().min(100);
                if params.enrich && enrichment_pool_size > 0 {
                    // First pass: collection lookup for franchise-style queries.
                    ...
                    for idx in 0..enrichment_pool_size {
                        ...
                        if let Some(tmdb_match) = collection_lookup.get(&normalized) {
                            ...
                            // Run per-item enrichment in parallel for non-collection candidates.
                            ...
                            enrichment_tasks.push(tokio::spawn(async move {
                                ...
                                // Retry without year when the strict year query does not find a match.
                                if year.is_some() {
                ...
                // Return a broad enriched set while keeping pagination metadata backward compatible.
                let total = results.len();
                let limit = params.limit.max(1).min(100);
                let page = params.page.max(1);
                let total_pages = (total + limit - 1) / limit;
                
                // FHUB currently lets the frontend perform final paging/grouping over this enriched set.
                Json(SearchResponse {
                    results,
                    page,
                    limit,
        ...
        Err(_) => Json(SearchResponse { results: vec![], page: 1, limit: 20, total: 0, total_pages: 0, has_more: false, version: "fhub-search-error".to_string() }),
    }
}
...
fn apply_tmdb_data(res: &mut SearchResult, data: &Value) {
    ...
    if let Some(media_type) = data["media_type"].as_str() {
        ...
        // Type-specific metadata searches may not include media_type, so infer from known date fields.
    ...
    // Release date and year normalization
