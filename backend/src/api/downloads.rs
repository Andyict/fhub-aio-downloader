...
use crate::utils::{parser::FilenameParser, status_utils::StatusCounts};
use crate::hosts::{base::HostHandler, fshare::FshareHandler};
...
async fn fetch_fshare_folder_page(
    ...
    token: Option<&str>,
    page: u32,
) -> Result<Vec<FshareFolderApiItem>, (StatusCode, String)> {
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|e| (StatusCode::BAD_GATEWAY, format!("Không gọi được Fshare folder API: {}", e)))?;

    if !response.status().is_success() {
        return Err((
            StatusCode::BAD_GATEWAY,
            format!("Fshare folder API trả về HTTP {}", response.status()),
        ));
    }
...
async fn collect_fshare_folder_items(
    ...
    recursive: bool,
    depth: u32,
) -> Result<Vec<PreviewFolderItem>, (StatusCode, String)> {
    if depth > 8 {
        return Ok(Vec::new());
    }

    let folder_code = extract_folder_code(folder_url)
        .ok_or((StatusCode::BAD_REQUEST, "URL này không phải Fshare folder hợp lệ".to_string()))?;
    let token = extract_folder_token(folder_url);

    let mut page = 1u32;
    let mut collected = Vec::new();

    loop {
        let page_items = fetch_fshare_folder_page(client, &folder_code, token.as_deref(), page).await?;
        ...
        for item in page_items {
            ...
            collected.push(PreviewFolderItem {
                ...
                quality: parsed_name.quality_attrs.quality_name(),
            });

            if recursive && is_directory {
                let mut nested = Box::pin(collect_fshare_folder_items(client, &url, true, depth + 1)).await?;
                collected.append(&mut nested);
            }
        }
...
pub async fn preview_link_download(
    ...
    if resolved_url.contains("/folder/") {
        return preview_folder_download(Json(PreviewFolderRequest {
            url: payload.url,
            recursive: payload.recursive,
        })).await;
    }

    if !resolved_url.contains("/file/") {
        return Err((StatusCode::BAD_REQUEST, "URL này không phải FShare file hoặc folder hợp lệ".to_string()));
    }

    let handler = FshareHandler::new(state.config.fshare.clone(), crate::hosts::create_shared_client());
    let info = tokio::time::timeout(
        std::time::Duration::from_secs(15),
        handler.get_file_info(&resolved_url),
    )
    .await
    .map_err(|_| (StatusCode::GATEWAY_TIMEOUT, "Kiểm tra link FShare quá thời gian".to_string()))?
    .map_err(|e| (StatusCode::BAD_GATEWAY, format!("Không lấy được thông tin file FShare: {}", e)))?;

    let parsed_name = FilenameParser::parse(&info.filename);
    let item = PreviewFolderItem {
    ...
    Ok(Json(PreviewFolderResponse {
        success: true,
        original_url: payload.url,
        resolved_url,
        folder_code: String::new(),
...
pub async fn preview_folder_download(
    ...
    let resolved_url = normalize_external_url(&payload.url);
    let folder_code = extract_folder_code(&resolved_url)
        .ok_or((StatusCode::BAD_REQUEST, "URL này không phải Fshare folder hợp lệ".to_string()))?;
    ...
    let client = reqwest::Client::builder()
        ...
        .build()
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Không tạo được HTTP client: {}", e)))?;

    let first_page = fetch_fshare_folder_page(&client, &folder_code, extract_folder_token(&resolved_url).as_deref(), 1).await?;
    ...
    let items = collect_fshare_folder_items(&client, &resolved_url, recursive, 0).await?;
    let file_count = items.iter().filter(|i| !i.is_directory).count();
    let total_size = items.iter().filter(|i| !i.is_directory).map(|i| i.size).sum();
