...
//!
//! Provides Newznab-compatible endpoints for integration with library services.
//! This allows NAS applications to search for content on Fshare.
...
struct NzbDownloadParams {
    ...
    season: Option<u32>,
    episode: Option<u32>,
    // Category from library services (e.g., "tv-tvlib", "movies")
    cat: Option<String>,
}

/// Generate a fake NZB file for library services validation
...
<nzb xmlns="http://www.newznab.com/DTD/2010/nzb">
    <head>
{}    </head>
    <file poster="FHub" date="1706534400" subject="Fshare_Download_{}">
    ...
    if let Some(groups) = smart_response.groups {
        for group in groups {
            for file in group.files {
                ...
                // Synthesize title for TV library parsing
...
/// Synthesize a parseable title for TV library
...
/// Generate NZB download URL for library services
...
async fn handle_movie_search(
    ...
    let year = None; // Movie library doesn't typically send year as separate param
...
fn write_item<W: std::io::Write>(writer: &mut quick_xml::Writer<W>, result: IndexerResult) {
    ...
    // Enclosure (CRITICAL for NAS suite to recognize downloadable content)
    ...
    // Newznab attributes (critical for NAS suite)
    ...
    // Parent category attribute (required by NAS suite)
    // 2000 = Movies, 5000 = TV
    ...
    // Size attribute (NAS suite prefers this over <size> tag)
