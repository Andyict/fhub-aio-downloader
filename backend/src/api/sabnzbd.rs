...
//!
//! Provides SABnzbd-compatible endpoints for integration with library services.
//! This allows NAS applications to use FHub as a download client.
...
async fn handle_add_file(
    ...
    // Extract category from params (TV library sends it as query param)
...
/// Get full status (flattened queue response for NAS compatibility)
...
/// Get history
/// 
/// Returns completed/failed downloads for library services to detect and import.
/// Uses TMDB title for the `name` field so NAS can match to series/movies in its library.
...
async fn handle_history(state: Arc<AppState>) -> Result<Json<serde_json::Value>, StatusCode> {
    ...
    let slots: Vec<SabHistorySlot> = tasks.iter()
        ...
        .filter_map(|t| {
            ...
            // which TV library rejects as "not a valid path".
            if t.destination.is_empty() {
                tracing::debug!("SABnzbd history: skipping {} - destination is empty", t.filename);
                return None;
            }

            // Map container path to TV library path.
            // FHub container:  /downloads/Show/ep.mkv
            // TV library (LXC 110):   /data/downloads/Show/ep.mkv  (has /data → /data)
            let tvlib_path = if t.destination.starts_with("/downloads/") {
                // Container-internal path → prefix with /data
                format!("/data{}", t.destination)
            } else if t.destination.starts_with("/data/downloads/") {
                // Already the correct TV library-visible path (tasks added post-fix)
                t.destination.clone()
            } else {
                // Unknown path — log and skip so TV library doesn't get a garbage path
                tracing::warn!(
            ...
            let storage = if let Some(parent) = std::path::Path::new(&tvlib_path).parent() {
                parent.to_string_lossy().to_string()
            } else {
                tvlib_path.clone()
            };
            
            // Build a name that library services can reliably parse for series/movie matching.
            // Use scene-release-like format: "Series.Name.S01E07.WEB-DL.FHub"
            // This strips special chars, replaces spaces with dots, matching scene naming
            // conventions that TV library's parser is optimized for.
            let name = if let Some(ref tmdb_title) = t.tmdb_title {
                ...
                if let (Some(season), Some(episode)) = (t.tmdb_season, t.tmdb_episode) {
                    // TV: "Series.Name.S01E07.WEB-DL.FHub"
                    format!("{}.S{:02}E{:02}.WEB-DL.FHub", clean_title, season, episode)
                } else {
                    // Movie: "Movie.Name.WEB-DL.FHub"
                    format!("{}.WEB-DL.FHub", clean_title)
                }
            } else {
...
/// Get config (for NAS compatibility testing)
/// 
/// library services use complete_dir + category dir to determine where downloads land.
/// Our downloads go directly into series/movie-named folders under /data/downloads/,
/// so category dirs must be empty to avoid TV library looking in /data/downloads/TV/ etc.
