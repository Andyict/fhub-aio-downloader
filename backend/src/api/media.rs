...
pub struct MediaItemResponse {
    #[serde(flatten)]
    pub item: MediaItem,
    /// Number of FHUB activities associated with this media item
...
/// An FHUB activity instance with quality info for the grouped response
#[derive(Serialize)]
pub struct DownloadInstance {
    ...
    /// TV library series_id when this activity is linked to the TV library
    pub arr_series_id: Option<i64>,
    /// Movie library movie_id when this activity is linked to the movie library
    pub arr_movie_id: Option<i64>,
}

/// FHUB activities grouped by episode key (e.g., "S01E01") or "movie"
#[derive(Serialize)]
pub struct MediaDownloadsResponse {
    pub tmdb_id: i64,
    pub media_type: String,
    pub title: String,
    /// Map of episode key to list of FHUB activity instances
    /// Keys: "S01E01", "S02E03", "movie" for movies
    pub episodes: HashMap<String, Vec<DownloadInstance>>,
    /// Total number of FHUB activities across all episodes
...
/// GET /api/media — List all media items with FHUB activity counts
async fn list_media(
    State(state): State<Arc<AppState>>,
) -> Result<Json<Vec<MediaItemResponse>>, (axum::http::StatusCode, String)> {
    let items = state.db.get_all_media_items_async().await
        .map_err(|e| (axum::http::StatusCode::INTERNAL_SERVER_ERROR, format!("FHUB database error: {}", e)))?;
...
/// GET /api/media/:tmdb_id — Get a single media item with episodes and FHUB activities
async fn get_media_detail(
    ...
    match result {
        ...
        None => Err((axum::http::StatusCode::NOT_FOUND, format!("FHUB media item with TMDB ID {} was not found", tmdb_id))),
    }
}
...
/// GET /api/media/:tmdb_id/downloads — Get FHUB activities grouped by episode with quality info.
/// Always returns 200, even if no media_items row exists yet, with an empty episodes map.
/// Activities are read directly from the `downloads` table by tmdb_id, so this works
/// before any MediaItem is created and while items are still pending.
async fn get_media_downloads(
    ...
    let (media_type, title) = if let Ok(Some(item)) = state.db.get_media_item_async(tmdb_id).await {
        ...
        // Media item does not exist yet; still return activities by TMDB ID.
        ("unknown".to_string(), tmdb_id.to_string())
    };

    // Step 2: Query activities directly by tmdb_id without requiring a media_items join.
