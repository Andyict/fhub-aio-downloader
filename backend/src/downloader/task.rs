...
/// Media type for FHub integration routing
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
pub enum MediaType {
...
pub struct DownloadTask {
    ...
    /// TMDB metadata for FHub matching
    ...
    /// FHub integration IDs (populated by webhook when series/movie is added)
    pub arr_series_id: Option<i64>,    // FHub series ID
    pub arr_movie_id: Option<i64>,     // FHub movie ID
...
impl DownloadTask {
    ...
    /// Detect media type for FHub integration routing
    /// 
    /// Logic:
    /// - Has tmdb_season + tmdb_episode → TvEpisode (single episode)
    /// - Has batch_id → TvSeries (batch of episodes)
    /// - Category contains "movie" or "fhub" → Movie
    /// - Default: infer from category (tv/fhub → TvEpisode, else → Movie)
    pub fn detect_media_type(&self) -> MediaType {
        ...
        if category_lower.contains("movie") || category_lower.contains("fhub") {
            return MediaType::Movie;
        }
        
        if category_lower.contains("tv") || category_lower.contains("fhub") {
