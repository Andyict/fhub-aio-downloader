...
impl FolderCacheService {
    ...
    pub async fn sync_all_sources(&self) -> anyhow::Result<SyncReport> {
        ...
        // Get the gist URL from settings. If it is missing/invalid/unreachable,
        // keep FHub self-contained by falling back to the bundled folder list.
        ...
        // Fetch the gist as plain text (one URL per line), or read bundled fallback.
        let client = reqwest::Client::builder()
            .timeout(std::time::Duration::from_secs(30))
            .build()?;

        let fallback_paths = [
            "/app/fshare_folder_list.txt",
            "fshare_folder_list.txt",
            "../fshare_folder_list.txt",
        ];

        let read_bundled = || -> anyhow::Result<String> {
            for path in fallback_paths {
                if let Ok(body) = std::fs::read_to_string(path) {
                    tracing::warn!("[FOLDER-CACHE] Using bundled folder source list: {}", path);
                    return Ok(body);
                }
            }
            Err(anyhow::anyhow!("No bundled fshare_folder_list.txt found"))
        };

        let body = if gist_url.trim().is_empty() {
            errors.push("No gist URL configured; using bundled fshare_folder_list.txt".to_string());
            read_bundled()?
        } else {
            match client.get(&gist_url).send().await {
                Ok(resp) => {
                    if resp.status().is_success() {
                        resp.text().await?
                    } else {
                        let msg = format!("Gist fetch failed: HTTP {}; using bundled fshare_folder_list.txt", resp.status());
                        errors.push(msg.clone());
                        tracing::warn!("[FOLDER-CACHE] {}", msg);
                        read_bundled()?
                    }
                }
                Err(e) => {
                    let msg = format!("Gist fetch error: {}; using bundled fshare_folder_list.txt", e);
                    errors.push(msg.clone());
                    tracing::warn!("[FOLDER-CACHE] {}", msg);
                    read_bundled()?
                }
