...
use crate::error::{FHubError, FHubResult};
...
impl DownloadService {
    ...
    pub async fn get_task(&self, id: Uuid) -> FHubResult<DownloadTask> {
        self.orchestrator
            .get_task_unified(id)
            .await
            .ok_or(FHubError::DownloadNotFound(id))
    }
    
    /// Pause a single task
    pub fn pause_task(&self, id: Uuid) -> FHubResult<DownloadTask> {
        self.orchestrator
            .task_manager()
            .pause_task(id)
            .ok_or(FHubError::DownloadInvalidState {
                id,
    ...
    pub fn resume_task(&self, id: Uuid) -> FHubResult<DownloadTask> {
        self.orchestrator
            .task_manager()
            .resume_task(id)
            .ok_or(FHubError::DownloadInvalidState {
    ...
    /// Retry a failed task
    pub fn retry_task(&self, id: Uuid) -> FHubResult<DownloadTask> {
        self.orchestrator
            .task_manager()
            .retry_task(id)
            .ok_or(FHubError::DownloadInvalidState {
    ...
    /// Delete a task
    pub fn delete_task(&self, id: Uuid) -> FHubResult<()> {
        ...
        if deleted {
            ...
            Err(FHubError::DownloadNotFound(id))
        }
    }
    ...
    pub async fn get_batch_tasks(&self, batch_id: &str) -> FHubResult<Vec<DownloadTask>> {
        let tasks = self.db
            .get_tasks_by_batch_id_async(batch_id.to_string())
            .await
            .map_err(|e| FHubError::Database(e.to_string()))?;
        
        if tasks.is_empty() {
            return Err(FHubError::BatchNotFound(batch_id.to_string()));
        }
    ...
    pub async fn get_batch_stats(&self, batch_id: &str) -> FHubResult<BatchStats> {
        let tasks = self.get_batch_tasks(batch_id).await?;
        Ok(BatchStats::from_tasks(&tasks))
    }
    
    /// Pause all tasks in a batch
    pub async fn pause_batch(&self, batch_id: &str) -> FHubResult<usize> {
        let tasks = self.db
            .get_tasks_by_batch_id_async(batch_id.to_string())
            .await
            .map_err(|e| FHubError::Database(e.to_string()))?;
    ...
    pub async fn resume_batch(&self, batch_id: &str) -> FHubResult<usize> {
        let tasks = self.db
            .get_tasks_by_batch_id_async(batch_id.to_string())
            .await
            .map_err(|e| FHubError::Database(e.to_string()))?;
    ...
    pub async fn delete_batch(&self, batch_id: &str) -> FHubResult<usize> {
        let tasks = self.db
            .get_tasks_by_batch_id_async(batch_id.to_string())
            .await
            .map_err(|e| FHubError::Database(e.to_string()))?;
