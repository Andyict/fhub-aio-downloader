//! FHub Metadata Service
//!
//! Centralized service for cinema metadata API interactions.
//! Keeps FHUB metadata enrichment, artwork lookup, and title aliases in one boundary.
...
/// Alternative title from the metadata provider
...
/// Translation from the metadata provider
#[derive(Debug, Clone)]
...
/// Media enrichment data for FHUB matching
...
pub struct MediaEnrichment {
    ...
    /// Official title/name used for FHUB display and matching
    pub official_name: Option<String>,
    /// Original title in the content origin language
    pub original_name: Option<String>,
    /// BCP-47 language code for the original language
...
/// FHUB metadata service for centralized API access
#[derive(Clone)]
...
impl TmdbService {
    /// Create a new metadata service with shared HTTP client
    pub fn new(client: Arc<Client>) -> Self {
        Self { client }
    }

    /// Create with a new HTTP client configured for FHUB metadata performance
    ...
    async fn get(&self, path: &str) -> Option<Value> {
        ...
        match self.client.get(&url).send().await {
            ...
            Ok(resp) => {
                warn!("FHUB metadata API returned {}: {}", resp.status(), path);
                None
            }
            Err(e) => {
                warn!("FHUB metadata API request failed: {} - {}", path, e);
                None
            }
        }
    }
    ...
    /// Get full TV enrichment in one metadata call using append_to_response.
    pub async fn get_tv_enrichment(&self, tmdb_id: i64) -> MediaEnrichment {
        let mut enrichment = MediaEnrichment::default();

        // Single metadata call: details + alternative_titles + translations
    ...
    /// Get full movie enrichment in one metadata call using append_to_response.
    pub async fn get_movie_enrichment(&self, tmdb_id: i64) -> MediaEnrichment {
        let mut enrichment = MediaEnrichment::default();

        // Single metadata call: details + alternative_titles + belongs_to_collection
        let data = self.get_with_params(
            &format!("/movie/{}", tmdb_id),
    ...
    /// Find by external ID such as IMDB or TVDB
