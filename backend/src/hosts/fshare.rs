...
impl FshareHandler {
    ...
    async fn perform_login(&self) -> anyhow::Result<SessionData> {
        ...
        // FShare can occasionally return a transient `405 Authenticate fail` on
        // the first request after credentials/session are changed, while the
        // same credentials succeed moments later. Retry once before falling
        // back so first-time account add can verify VIP immediately.
        let mut last_api2_error: Option<anyhow::Error> = None;
        for attempt in 1..=2 {
            match self.perform_api2_login(&email, &password).await {
                Ok(session) => return Ok(session),
                Err(e) => {
                    let err_str = format!("{}", e);
                    let transient_auth = err_str.contains("Authenticate fail")
                        || err_str.contains("405")
                        || err_str.contains("request failed")
                        || err_str.contains("timed out");

                    if attempt == 1 && transient_auth {
                        tracing::warn!(
                            "[FSHARE] Secondary API login failed transiently; retrying once before web fallback: {}",
                            err_str
                        );
                        tokio::time::sleep(std::time::Duration::from_secs(2)).await;
                        last_api2_error = Some(e);
                        continue;
                    }
    ...
    /// Resolve public/guest download URL via the web interface without account login.
    /// Real FShare form: `<form id="form-download" action="/download/get" method="post">`
    /// Fields: `_csrf-app`, `linkcode`, `ushare`, `withFcode5`, optionally `password`/`des`.
    /// Response currently returns JSON: `{ "url": "https://download...", "name": ..., "wait_time": ... }`.
    async fn resolve_download_url_guest(&self, url: &str) -> anyhow::Result<ResolvedUrl> {
        tracing::info!("[FSHARE-GUEST] Resolving public download URL via /download/get: {}", url);

        let path_part = url.split("/file/").last().unwrap_or("");
        let fcode = path_part.split('?').next().unwrap_or("").trim_matches('/');
        if fcode.is_empty() {
            return Err(anyhow::anyhow!("[FSHARE-GUEST] Could not extract file code from URL"));
        }

        let des_token = url.split('?')
            .nth(1)
            .and_then(|qs| qs.split('&').find(|p| p.starts_with("des=") || p.starts_with("token=")))
        ...
        let file_page_resp = self.web_client
            ...
            .send()
            .await
            .map_err(|e| anyhow::anyhow!("[FSHARE-GUEST] File page GET failed: {}", e))?;

        let page_cookies = Self::extract_cookies(&file_page_resp);
        let html = file_page_resp.text().await.unwrap_or_default();
        let csrf = Self::extract_csrf_token(&html)
            .ok_or_else(|| anyhow::anyhow!("[FSHARE-GUEST] Could not find CSRF token on file page"))?;
        ...
        let resp = self.web_client
            ...
            .send()
            .await
            .map_err(|e| anyhow::anyhow!("[FSHARE-GUEST] Download form POST failed: {}", e))?;
        ...
        if let Ok(data) = serde_json::from_str::<Value>(&body) {
            ...
            {
                if !url_str.is_empty() {
                    tracing::info!("[FSHARE-GUEST] Got public direct URL for {}", fcode);
            ...
            tracing::warn!("[FSHARE-GUEST] JSON response without direct URL: {}", &body[..body.len().min(300)]);
        }

        Err(anyhow::anyhow!("[FSHARE-GUEST] No guest download URL found (status {}): {}", status, &body[..body.len().min(300)]))
    }
    ...
    async fn fetch_account_status_for_session(&self, session: &SessionData) -> anyhow::Result<AccountStatus> {
        ...
        tracing::info!("[FSHARE] Account status verified: type='{}', level='{}', is_premium={}, valid_until={:?}", account_type, level, is_premium, valid_until);
...
impl HostHandler for FshareHandler {
    ...
    async fn resolve_download_url(&self, url: &str) -> anyhow::Result<ResolvedUrl> {
        ...
        // Try public guest resolution first. This handles publicly shared FShare files
        // without requiring account credentials and mirrors the Kodi addon/web form flow.
        match self.resolve_download_url_guest(url).await {
            Ok(resolved) => return Ok(resolved),
            Err(e) => tracing::info!("[FSHARE-GUEST] Guest resolve unavailable, falling back to account/API flow: {}", e),
        }
