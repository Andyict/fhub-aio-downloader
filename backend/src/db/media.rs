...
//! universal join key across downloads, NAS integration, and the frontend.

use serde::{Deserialize, Serialize};

/// A media entity (movie or TV series) identified by TMDB ID.
/// This is the central record that links downloads to NAS library state.
...
pub struct MediaItem {
    ...
    // ── FHub integration state ──────────────────────────────────────────
    /// FHub series_id or FHub movie_id
    pub arr_id: Option<i32>,
    /// "fhub" or "fhub"
    pub arr_type: Option<String>,
    /// File system path in NAS library
    ...
    /// FHub: does file exist on disk?
    pub arr_has_file: bool,
    /// Bytes on disk (from NAS)
    ...
    /// Last time NAS data was synced for this item
...
impl MediaItem {
    ...
    /// Check if this media item is linked to an NAS instance
