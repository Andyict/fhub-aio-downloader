...
mod tests {
    ...
    #[test]
    fn compare_parsers() {
        // Load dataset from JSON when the optional local fixture is available.
        use std::fs::File;
        ...
        let file_path = std::env::var("FHUB_DATASET1_FIXTURE")
            .unwrap_or_else(|_| "debug_log/dataset_1.json".to_string());

        if !Path::new(&file_path).exists() {
            eprintln!("Skipping parser comparison; fixture not found at {}", file_path);
            return;
        }
