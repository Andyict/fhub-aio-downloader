<script lang="ts">
  ...
  // Saved integration
  let libraryMovie = $state<TrackedMovie | null>(null);
    ...
    try {
      ...
      const next = exists
        ...
        : [{
            ...
            mediaType: "movie",
            releaseDate: getYear(movie.release_date),
            originalFilename: movie.original_title || movie.title,
          }, ...items].slice(0, 100);
  ...
  async function handleAddToLibrary() {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        toasts.success(`"${movie.title}" saved`);
        // Mark as in-library optimistically so the button disappears
        libraryMovie = {
          id: data.arr_id ?? Number(movieId),
          title: movie.title,
          tmdb_id: Number(movieId),
        } as any;
      } else {
        ...
        <section class="overview-section">
          ...
          {:else}
            <p class="overview-text">
              {movie?.overview ||
                "Overview is not available yet. FHub will show more details when metadata is updated."}
            </p>
          {/if}
        </section>
      ...
      <aside class="detail-sidebar">
        ...
        <div class="stats-panel glass-panel">
          ...
          {:else if movie}
            ...
            {#if releaseInfo}
              <div class="info-section-label">{uiLanguage === "vi" ? "Lịch phát hành" : "Release Dates"}</div>
            ...
            {:else}
              <div class="info-row">
                <span class="label">{uiLanguage === "vi" ? "Ngày phát hành" : "Release Date"}</span>
                <span class="value"
            ...
            <div class="info-section-label">{uiLanguage === "vi" ? "Doanh thu" : "Financials"}</div>
            {#if movie.budget > 0}
              <div class="info-row">
                <span class="label">{uiLanguage === "vi" ? "Ngân sách" : "Budget"}</span>
                <span class="value">{formatCurrency(movie.budget)}</span>
              </div>
            {/if}
            {#if movie.revenue > 0}
              <div class="info-row">
                <span class="label">{uiLanguage === "vi" ? "Doanh thu" : "Revenue"}</span>
              ...
              <div class="info-row">
                ...
                <span
                  ...
                  style="color: {libraryMovie.monitored
                    ...
                    : '#94a3b8'}"
                >
                  {libraryMovie.monitored ? (uiLanguage === "vi" ? "Có" : "Yes") : (uiLanguage === "vi" ? "Không" : "No")}
                </span>
              </div>
              <div class="info-row">
                <span class="label">{uiLanguage === "vi" ? "Tệp" : "File Status"}</span>
                <span
                  class="value"
                  style="color: {libraryMovie.hasFile ? '#34d399' : '#ef4444'}"
                >
                  {libraryMovie.hasFile ? (uiLanguage === "vi" ? "Đã có" : "Acquired") : (uiLanguage === "vi" ? "Thiếu" : "Missing")}
                </span>
              </div>
...
<style>
  ...
  .collection-banner:hover {
    ...
    transform: translateY(-2px);
    box-shadow:
      0 10px 40px rgba(255, 138, 31, 0.15),
      0 0 20px rgba(255, 138, 31, 0.05);
  }
