<script lang="ts">
  ...
  import { fetchAllMovies, type TrackedMovie } from "$lib/stores/libraryBridge";
  ...
  // Saved data
  let allMovies = $state<TrackedMovie[]>([]);
  ...
  function handleSmartSearch(movie: TMDBMovie) {
    // Auto-save into the saved list if not already tracked (fire-and-forget)
    if (!libraryTmdbIds.has(movie.id)) {
      saveMovieSilently(movie);
    }
    ui.openSmartSearch({
  ...
  /** Silently add a movie to the saved list without blocking the UI */
  async function saveMovieSilently(movie: TMDBMovie) {
    try {
  ...
  async function handleAddToLibrary(movie: TMDBMovie) {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        toasts.info(`"${movie.title}" is already in your movie library`);
      } else {
...
<style>
  ...
  .section-badge {
    ...
    top: 0;
    right: 2rem;
    background: var(--color-primary, #ff8a1f);
    color: #000;
    font-family: var(--font-mono, monospace);
    font-size: 0.55rem;
    font-weight: 900;
  ...
  .section-label::after {
    ...
    flex: 1;
    height: 1px;
    background: linear-gradient(to right, rgba(255, 138, 31, 0.2), transparent);
  }
  .overview-text {
    font-size: 1rem;
    line-height: 1.8;
  ...
  .col-card:hover .col-card-inner {
    ...
    box-shadow: 0 20px 50px -10px rgba(255, 138, 31, 0.25);
  }
  ...
  .card-actions :global(.media-action-btn:first-child) {
    flex: 1;
  }
