<script lang="ts">
  ...
  function latestTrackItems(track: Track) {
    const detail = details[track.id];
    if (!detail?.items?.length) return [];
    return sortedTrackItems(detail.items)
      .filter((item) => itemEpisodeNumber(item) || item.completed_at || item.queued_at || item.first_seen_at)
      .slice(0, 1);
  }
      ...
      <article class:off={!track.enabled} class:expanded onclick={() => toggleDetails(track)} onkeydown={(event) => { if (event.key === "Enter" || event.key === " ") void toggleDetails(track); }} role="button" tabindex="0" aria-expanded={expanded}>
        <div class="card-head">
          ...
          <div>
            ...
            {#each [latestTrackItems(track)] as latestItems}
              {#if latestItems.length}
                <div class="latest-episodes" aria-label="Tập mới nhất">
                  {#each latestItems as item}
                    <span title={item.file_name}>
                      <span class="material-icons">play_circle</span>
                      <b>{episodeName(item)}</b>
                      <small>{compactTrackTime(item.completed_at || item.queued_at || item.first_seen_at)}</small>
                    </span>
                  {/each}
                </div>
              {/if}
