<script lang="ts">
  ...
  let chromaR: HTMLImageElement; // violet-indigo clone
  let chromaB: HTMLImageElement; // violet-slate clone
  ...
  const TITLE = "FHUB";
  const SUBTITLE = "MEDIA AUTOMATION SUITE";
    ...
    tl.add(
      {
        ...
        filter: [
          ...
          "brightness(1) blur(0px) drop-shadow(0 0 30px rgba(255, 138, 31, 0.8))",
        ],
        duration: 500,
        easing: "easeOutExpo",
        begin() {
  ...
  function startParticleEmitter() {
    ...
    const DOT_COLORS = ["#8b5cf6", "#8b5cf6", "#8b5cf6", "#a78bfa", "#818cf8"];
  ...
  function playIdle() {
    ...
    idleAnims.push(
      anime({
        targets: logoImg,
        filter: [
          "drop-shadow(0 0 20px rgba(255, 138, 31, 0.7)) drop-shadow(0 0 60px rgba(255, 138, 31, 0.3))",
          "drop-shadow(0 0 30px rgba(255, 138, 31, 0.9)) drop-shadow(0 0 80px rgba(255, 138, 31, 0.5))",
        ],
        duration: 2000,
        direction: "alternate",
        loop: true,
...
<div class="intro-overlay" bind:this={overlay}>
  ...
  <div class="logo-wrap">
    ...
    <img
      src="/images/fhub-intro-logo.png"
      alt=""
  ...
  <!-- Title hidden: logo-only intro -->
  <div class="title-wrap hidden-title" bind:this={titleWrap} aria-hidden="true">
...
<style>
  ...
  .bg-void {
    position: absolute;
    inset: 0;
    /* Primary halftone dot field — violet dots radiating from center */
  ...
  .glow-orb {
    ...
    inset: 0;
    margin: auto;
    width: min(105vmin, 980px);
    height: min(105vmin, 980px);
    border-radius: 50%;
    background: radial-gradient(
      circle,
      rgba(255, 138, 31, 0.38) 0%,
      rgba(0, 150, 200, 0.16) 40%,
      transparent 70%
    );
    filter: blur(52px);
    pointer-events: none;
  ...
  .ring {
    ...
    margin: auto;
    border-radius: 50%;
    border: 2px solid rgba(255, 138, 31, 0.6);
    pointer-events: none;
  }
  .ring-inner {
    ...
    border-width: 2px;
    box-shadow:
      0 0 20px rgba(255, 138, 31, 0.4),
      inset 0 0 20px rgba(255, 138, 31, 0.1);
  }
  .ring-outer {
    width: min(112vmin, 1040px);
    height: min(112vmin, 1040px);
    border-width: 1px;
    border-color: rgba(255, 138, 31, 0.3);
    box-shadow: 0 0 30px rgba(255, 138, 31, 0.2);
  }
  ...
  .scan-line {
    ...
    box-shadow: 0 0 8px rgba(255, 138, 31, 0.4);
    pointer-events: none;
    opacity: 0;
  }
  ...
  .particle-box :global(.particle) {
    ...
    box-shadow:
      0 0 4px var(--pcolor, #8b5cf6),
      0 0 8px var(--pcolor, rgba(255, 138, 31, 0.3));
  ...
  .title-main {
    ...
    text-align: center;
    text-shadow:
      0 0 20px rgba(255, 138, 31, 0.8),
      0 0 40px rgba(255, 138, 31, 0.4);
  }
  .title-sub {
