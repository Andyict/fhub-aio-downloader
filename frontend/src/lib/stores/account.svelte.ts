...
class AccountStore {
    ...
    async switchAccount(newEmail: string, newPassword: string): Promise<any> {
        const old = this.primary;
        if (old?.email && old.email !== newEmail) {
            const removed = await settingsStore.removeAccount(old.email);
            if (removed?.success === false) return removed;
        }
